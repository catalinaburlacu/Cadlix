        ...
        <section className='history-list' aria-label='Viewing history'>
          {historyEntries.length === 0 ? (
            ...
            historyEntries.map(entry => (
              <Link key={entry.id} to={`/movie/${entry.id}`} className='history-card'>
                ...
                <div className='history-meta'>
                  <p>
                    <strong>Series:</strong> {entry.series || '-'}
                  </p>
                  <p>
                    <strong>Episode:</strong> {entry.episode || '-'}
                  </p>
                  <p>
                    <strong>Moment:</strong> {entry.progress || '-'}
                  </p>
                  <p>
                    <strong>Watched At:</strong> {formatWatchDate(entry.watchedAt)}
                  </p>
                </div>
              </Link>
            ))
