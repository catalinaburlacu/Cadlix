...
import { PLANS_DATA } from '../../../mocks/subscriptions'
import type { PlanData } from '@/types'
import './Subscriptions.css'

interface PlanWithAction extends PlanData {
  onSelect: () => void
}
...
export default function Subscriptions() {
  ...
  const plans: PlanWithAction[] = PLANS_DATA.map(planData => ({
    ...planData,
    onSelect:
      planData.id === 'free'
        ? () => {
  ...
  const toggleDetails = (planId: string): void => {
        ...
        <div className='pricing-table'>
          {plans.map(plan => {
            ...
            return (
              <div
                key={plan.id}
                className={`pricing-option ${plan.id}${isExpanded ? ' expanded' : ''}`}
              >
                <h1>{plan.name}</h1>
