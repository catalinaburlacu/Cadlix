...
import type { VideoItem, WatchListItem } from '@/types'
import { MOCK_MEDIA_LIBRARY, MOCK_VIDEOS } from '../../../mocks/constants'
...
export default function VideoPlayer() {
  ...
  const didToastRef = useRef<boolean>(false)
  const [selectedListManual, setSelectedListManual] = useState<string>('')

  const videoItem: VideoItem | null = useMemo(
    () => MOCK_VIDEOS.find((item: VideoItem) => item.id === id) || null,
    [id],
  )

  const selectedList = useMemo(() => {
    const existing = (user?.watchList || []).find(
      (item: WatchListItem) => item.id === videoItem?.id,
    )
  ...
  const persistHistory = useCallback(() => {
    ...
    const duration = Math.max(
      1,
      Math.floor(videoRef.current?.duration || videoItem.durationSec || 1),
    )
    ...
    const computedStatus =
      watchedRatio >= 0.95 ? 'completed' : currentTime > 0 ? 'watching' : 'planned'
    const nextStatus = preserveStatus ? existingItem.status : computedStatus

    const baseItem: WatchListItem = existingItem || {
      id: videoItem.id,
      title: videoItem.title,
      category: videoItem.category,
      type: videoItem.type,
    ...
    const nextWatchList: WatchListItem[] = existingItem
  ...
  const handleAssignList = useCallback(() => {
    ...
    const baseItem: WatchListItem =
      (user.watchList || []).find((item: WatchListItem) => item.id === videoItem.id) ||
      MOCK_MEDIA_LIBRARY.find((item: WatchListItem) => item.id === videoItem.id) || {
        id: videoItem.id,
        title: videoItem.title,
        category: videoItem.category,
        type: videoItem.type,
    ...
    const nextWatchList: WatchListItem[] = watchList.some(item => item.id === updatedItem.id)
          ...
          <div className="video-list-controls">
            <select
              ...
              onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
                setSelectedListManual(e.target.value)
              }
            >
