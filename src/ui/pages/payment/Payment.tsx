...
import { ACCEPTED_ISSUERS, CARD_SCHEMES } from '../../../mocks/payment'
...
export default function Payment() {
  ...
  const { plan = 'Basic', price = 0 } = (location.state as LocationState) || {}
  ...
  const cleanPlan = (plan || 'Basic').replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27BF}]/gu, '').trim()
  ...
  function isSimpleSequence(number: string): boolean {
  ...
  function validateForm(): boolean {
    ...
    } else if (nameParts.some(part => part.length < 2 || !namePartPattern.test(part))) {
    ...
    const scheme: CardScheme | null =
      CARD_SCHEMES.find(
        item => item.lengths.includes(digitsOnlyCard.length) && item.pattern.test(digitsOnlyCard)
      ) || null
        ...
        <div className='payment-page'>
          ...
          <div className='paid-wrapper'>
            ...
            <div className='paid-message'>
              <h2>Plata reusita</h2>
              <p>
                Multumim! Ai achitat {price ? `$${price}` : 'suma'} pentru planul {cleanPlan}.
              </p>
              ...
              <div className='accepted-issuer-list'>
                {ACCEPTED_ISSUERS.map(issuer => (
                  <span key={issuer.name} className='accepted-chip' title={issuer.name}>
                    <span className='accepted-chip-icon' aria-hidden='true'>
                      {issuer.icon}
                    </span>
